	...
	switch (msg->getKind()) {
		...
		case WC_SIGNAL_START:{
			...
			if ((state != RX) || (changingToState != -1)) {
				...
				Radioinfos()<<"RX "<<self<<" "<<newSignal.ID<<" "<<-1<<" radio_not_in_RX_state";//ok
			...
			if ((RXmode->modulation == newSignal.modulation) && (newSignal.power_dBm >= RXmode->sensitivity))
				...
				if (newSignal.power_dBm < RXmode->sensitivity) {
					...
					Radioinfos()<<"RX "<<self<<" "<<newSignal.ID<<" "<<-1<<" below_sensitivity";//ok
				}
				else {
					stats.RxFailedModulation++;
					trace() << "Failed packet (WC_SIGNAL_START) from node " << newSignal.ID << ", wrong modulation";
					//Radioinfos()<<"RX "<<self<<" "<<newSignal.ID<<" "<<-1<<" wrong_modulation";
				}
			}
		...
		case WC_SIGNAL_END:{
			...
			if ((state != RX) || (changingToState != -1)) {
				if (endingSignal->bitErrors != ALL_ERRORS) {
					...
					Radioinfos()<<"RX "<<self<<" "<<signalID<<" "<<-1<<" no_RX_state";
				}
...
void Radio::handleRadioControlCommand(RadioControlCommand * radioCmd)
{
	switch (radioCmd->getRadioControlCommandKind()) {
		...
		case SET_CHANNEL:{
			...
			carrierFreq = ((radioCmd->getParameter() - 10)*5) + 2400;
			trace() << "Changed CARRIER_FREQ to " << carrierFreq << " MHz";
			trace() << "CHANNEL_SWITCH" << carrierFreq << " MHz";
...
void Radio::finishSpecific()
{
	...
	if (stats.RxReachedNoInterference > 0)
		collectOutput("RX pkt breakdown", "Received with NO interference", stats.RxReachedNoInterference);//ok
	if (stats.RxReachedInterference > 0)
		collectOutput("RX pkt breakdown", "Received despite interference", stats.RxReachedInterference);//ok
	if (stats.RxFailedNoInterference > 0)
		collectOutput("RX pkt breakdown", "Failed with NO interference", stats.RxFailedNoInterference);//ok
	if (stats.RxFailedInterference > 0)
		collectOutput("RX pkt breakdown", "Failed with interference", stats.RxFailedInterference);//ok
	if (stats.RxFailedNoRxState > 0)
		collectOutput("RX pkt breakdown", "Failed, non RX state", stats.RxFailedNoRxState);//ok
	if (stats.RxFailedSensitivity > 0)
		collectOutput("RX pkt breakdown", "Failed, below sensitivity", stats.RxFailedSensitivity);//ok
...
double Radio::popAndSendToWirelessChannel()
{
	...
	Radioinfos()<<"TX "<<self<<" "<<stats.transmissions;	
